...
def evaluate_model(llm, eval_data):
    ...
    for item in eval_data:
        ...
        try:
            completion = llm(
                prompt,
                max_tokens=len(target) + 50,  # 给一些余量
                temperature=0.1,
                stream=False
            )
...
def fine_tune():
    ...
    # 加载训练数据
    print("正在加载训练数据...")
    all_data = load_training_data("data/test_train.json")
    ...
    for epoch in range(EPOCHS):
        ...
        for item in progress_bar:
            current_step += 1
            current_lr = get_learning_rate(current_step, total_steps)
            ...
            try:
                ...
                if current_step % EVAL_STEPS == 0:
                    eval_loss = evaluate_model(llm, validation_data)
                    print(f"\n步骤 {current_step} 评估损失: {eval_loss:.4f}")
                    
                    # 如果是最佳模型，保存检查点
                    if eval_loss < best_eval_loss:
                        best_eval_loss = eval_loss
                        checkpoint_dir = os.path.join(OUTPUT_DIR, f'best_checkpoint')
    ...
    print(f"\n训练完成！最佳验证集损失: {best_eval_loss:.4f}")
